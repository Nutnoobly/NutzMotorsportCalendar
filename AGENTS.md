...
- **Ownership**: Agent authors and maintains all UI templates (`.templ`), styling (Tailwind), client interactions (HTMX/JS), and deployment pipelines (Fly.io, GHA).
...
| 9 | Dual-Role Split: Senior Go Mentor & Reviewer for backend (review-by-default, natural override); Active Developer for frontend & DevOps (Templ, HTMX, Tailwind, Fly.io, GHA); Contract-First handoff | Role Grilling (2026-09-16) |
...
- Roadmap steps: [done] 0 toolchain → [done] 1 migrations → [done] 2 sqlc/pgx queries
  ...
  → [next] 4 templ views → 5 htmx/countdown
  → 6 timezone JS → 7 refresh fetchers (Jolpica F1, Pulselive MotoGP) → 8 deploy (Fly.io + GHA cron).
- Resume point: Step 4 (templ views) — wire `views.Home` and `views.EventDetail` into `internal/web/handler.go`.
...
- (none — currently in build phase; executing Step 4: Templ views)

## Next Step Checklist (Step 4: Templ views)

- [x] Add `github.com/a-h/templ` dependency to `go.mod`
- [x] Create base layout template (`internal/views/layout.templ`) with Tailwind CSS & responsive shell
- [x] Create home page view (`internal/views/home.templ`) with series filter tabs & race weekend cards
- [x] Create event detail view (`internal/views/event_detail.templ`) for circuit, timetable, and podium
- [x] Compile templates with `templ generate` and verify build
- [ ] Connect `views.Home(...)` and `views.EventDetail(...)` into `internal/web/handler.go`
- [ ] Verify HTML rendering at `http://localhost:8080/`
